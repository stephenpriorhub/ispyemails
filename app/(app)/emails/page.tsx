...
export default async function EmailsPage({ searchParams }: { searchParams: Promise<Record<string, string>> }) {
  ...
  if (sp.q) {
    where.OR = [
      ...
      { fromName: { contains: sp.q, mode: "insensitive" } },
      { fromEmail: { contains: sp.q, mode: "insensitive" } },
    ];
  }
  ...
  return (
    <EmailList
      ...
      lists={lists}
      gurus={gurus}
      filters={{ publisherId: sp.publisher, topicId: sp.topic, placement: sp.placement, emailType: sp.type, search: sp.q, listId: sp.list, guruId: sp.guru, sortBy, order }}
    />
  );
}
