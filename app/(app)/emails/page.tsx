...
export default async function EmailsPage({ searchParams }: { searchParams: Promise<Record<string,string>> }) {
  ...
  const [emails,total,publishers,topics] = await Promise.all([
  ...
  return <EmailList emails={emails} total={total} page={page} pages={Math.ceil(total/limit)} publishers={publishers} topics={topics} filters={{publisherId:sp.publisher,topicId:sp.topic,placement:sp.placement,emailType:sp.type,search:sp.q,sortBy,order}} />;
}
