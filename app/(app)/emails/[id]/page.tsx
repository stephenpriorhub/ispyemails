...
export default async function EmailDetailPage({ params }: { params: Promise<{id:string}> }) {
  ...
  const [email, publishers, tags] = await Promise.all([
    prisma.email.findUnique({where:{id},include:{publisher:true,topics:{include:{topic:true}},tags:{include:{tag:true}},offer:true}}),
    prisma.publisher.findMany({orderBy:{name:"asc"},select:{id:true,name:true}}),
    prisma.tag.findMany({orderBy:{name:"asc"}}),
  ]);
  if (!email) notFound();
  return <EmailDetail email={email} publishers={publishers} allTags={tags} />;
}
