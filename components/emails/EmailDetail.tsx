  ...
  return (
    <div className="flex h-full">
      ...
      <div className="w-72 flex-shrink-0 overflow-y-auto bg-gray-900 space-y-4 p-4">
        ...
        <div><label className="text-xs text-gray-500 block mb-1.5">Publisher</label><select value={selectedPublisherId} onChange={e=>setSelectedPublisherId(e.target.value)} className="w-full py-1.5 px-2 bg-gray-800 border border-gray-700 rounded text-sm text-gray-300 focus:outline-none focus:border-amber-500"><option value="">Unassigned</option>{publishers.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select>{email.aiConfidence!=null&&!email.publisherConfirmed&&<p className="text-xs text-gray-600 mt-1">AI guess · {Math.round(email.aiConfidence*100)}% confidence</p>}</div>
        <div><label className="text-xs text-gray-500 block mb-1.5">Email Type</label><div className="grid grid-cols-2 gap-1">{EMAIL_TYPES.map(t=><button key={t} onClick={()=>setEmailType(t)} className={`text-xs py-1.5 px-2 rounded border transition-colors ${emailType===t?"bg-amber-500/10 border-amber-500/40 text-amber-400":"bg-gray-800 border-gray-700 text-gray-400 hover:border-gray-600"}`}>{t.replace("_"," ")}</button>)}</div></div>
        <div><label className="text-xs text-gray-500 block mb-1.5 flex items-center gap-1"><Hash className="w-3 h-3"/>Topics</label><div className="flex flex-wrap gap-1">{email.topics.map(({topic})=><div key={topic.id} className="flex items-center gap-1 bg-gray-800 px-2 py-0.5 rounded group"><span className="text-xs text-gray-300 capitalize">{topic.name}</span><button onClick={()=>ignoreTopic(topic.id)} className="text-gray-600 hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100 text-xs ml-1">✕</button></div>)}{email.topics.length===0&&<p className="text-xs text-gray-600">No topics detected</p>}</div></div>
        <div><label className="text-xs text-gray-500 block mb-1.5 flex items-center gap-1"><TagIcon className="w-3 h-3"/>Tags</label><div className="flex flex-wrap gap-1">{allTags.map(tag=>{const active=activeTagIds.includes(tag.id);return<button key={tag.id} onClick={()=>setActiveTagIds(active?activeTagIds.filter(id=>id!==tag.id):[...activeTagIds,tag.id])} className={`text-xs px-2 py-0.5 rounded border transition-colors ${active?"border-amber-500/40 text-amber-400 bg-amber-500/10":"border-gray-700 text-gray-500 bg-gray-800 hover:border-gray-600"}`}>{tag.name}</button>;})}</div></div>
